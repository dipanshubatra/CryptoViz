...
import { CrossChainBridgeAuditRecord } from '../../lib/CryptoCrossChainBridgeModel';
import { ShieldCheck, ShieldAlert, ExternalLink, Activity } from 'lucide-react';

interface TimelineProps {
  records: CrossChainBridgeAuditRecord[];
}

export const CryptoCrossChainBridgeTimeline: React.FC<TimelineProps> = ({ records }) => {
  return (
    <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Cross-Chain Bridge Routing Audit Ledger
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Real-time inter-chain telemetry, relayer execution status, and audit records.
          </p>
        </div>
        <span className="bg-slate-800 text-slate-300 font-semibold text-xs px-3 py-1 rounded-full border border-slate-700">
          {records.length} Bridge Executions
        </span>
      </div>
      ...
      {records.length === 0 ? (
        <div className="text-center py-10 border border-dashed border-slate-800 rounded-2xl">
          ...
          <p className="text-slate-400 text-sm font-medium">No cross-chain bridge records logged yet.</p>
        </div>
      ) : (
            ...
            >
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-white font-bold text-base">
                      {rec.sourceChain} ➔ {rec.targetChain} ({rec.assetSymbol})
                    </span>
                    <span
                  ...
                  <div className="text-xs text-slate-400 mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                    <span>Sent: ${rec.transferAmountUsd.toLocaleString()}</span>
                    <span>•</span>
                    <span>Fees: ${rec.totalFeePaidUsd}</span>
                    <span>•</span>
              ...
              <div className="text-right self-end sm:self-center w-full sm:w-auto pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-800/80">
                <div className="text-lg font-black text-cyan-400">
                  Net Rec: ${rec.netReceivedUsd.toLocaleString()}
                </div>
                <a
                  href={`https://etherscan.io/tx/${rec.executionTxHash}`}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 text-[11px] font-semibold text-cyan-400 hover:text-cyan-300 transition-colors mt-0.5 font-mono"
                >
